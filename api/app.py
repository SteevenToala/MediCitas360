...
# 4.5 POST /api/citas/<int:cita_id>/facturar - Retry generation of invoice (Protected)
@app.route("/api/citas/<int:cita_id>/facturar", methods=["POST"])
@require_token
def retry_facturar(cita_id):
    conn = get_db_connection()
    if conn is None:
        return jsonify({"error": "No se pudo conectar a la base de datos principal Supabase"}), 500
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT c.id, c.paciente, c.cedula, c.medico_id, c.fecha, c.hora, c.valor_pagar, 
                ...
                cur.execute("SELECT id, clave_acceso FROM facturas WHERE cita_id = %s;", (cita_id,))
                factura = cur.fetchone()
                if factura:
                    return jsonify({
                        "mensaje": "La factura ya existe para esta cita",
                        "clave_acceso": factura["clave_acceso"]
                    }), 200

                # 3. Call the independent billing service
                try:
                    invoice_payload = {
                        "cita_id": cita["id"],
                    ...
                    if bill_res.status_code == 200:
                        ...
                        cur.execute("""
                            ...
                            VALUES (%s, %s, %s, 'GENERADA', %s)
                            RETURNING id;
                        """, (cita["id"], cita["paciente"], float(cita["valor_pagar"]), clave_acceso))
                        factura_id = cur.fetchone()["id"]
                        conn.commit()

                        # 5. Save invoice to SQLite local database
                        local_factura = {
                            "id": factura_id,
                            "cita_id": cita["id"],
                            "paciente": cita["paciente"],
                        ...
                        return jsonify({
                            "error": f"El servicio de facturacion devolvio error {bill_res.status_code}: {bill_res.text}"
                        }), 502
                except Exception as ex_bill:
                    return jsonify({
                        "error": f"No se pudo conectar al servicio de facturacion XML: {str(ex_bill)}"
                    }), 503
    except Exception as e:
        return jsonify({"error": f"Error al procesar la facturacion: {str(e)}"}), 500
